...
def _vworld_headers() -> Dict[str, str]:
    return {
        ...
        "User-Agent": "urban-strategy/0.4.1",
...
def _fetch_vworld_parcel_candidates(target_geom) -> List[Dict[str, Any]]:
    ...
    for page in range(1, 11):
        ...
        resp = requests.get(VWORLD_DATA_URL, params=params, headers=_vworld_headers(), timeout=20)
        try:
...
def _vworld_official_land_area(pnu: str) -> Optional[float]:
    ...
    try:
        resp = requests.get(VWORLD_LAND_URL, params=params, headers=_vworld_headers(), timeout=15)
        resp.raise_for_status()
...
def vworld_test():
    ...
    try:
        resp = requests.get(VWORLD_DATA_URL, params=params, headers=_vworld_headers(), timeout=15)
        result = {
