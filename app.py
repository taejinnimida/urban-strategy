    ...
    if _database_url():
        ...
        with psycopg.connect(_database_url()) as conn:
            conn.execute("""
                ...
                (visitor_id, session_id, event_type, address_text, pnu_list,
            ...
            """, (
                data["visitor_id"], data.get("session_id"), data["event_type"],
...
def _analytics_rows(limit: int = 500) -> List[Dict[str, Any]]:
    if _database_url():
        ...
        with psycopg.connect(_database_url()) as conn:
            rows = conn.execute("""
                SELECT created_at, visitor_id, session_id, event_type, address_text,
                       pnu_list, area_m2, parcel_count, centroid_lat, centroid_lng,
                       recommendations, result_summary, user_agent_group
                FROM analytics_events ORDER BY created_at DESC LIMIT %s
            """, (limit,)).fetchall()
        keys = ["created_at","visitor_id","session_id","event_type","address_text","pnu_list","area_m2","parcel_count","centroid_lat","centroid_lng","recommendations","result_summary","user_agent_group"]
    ...
    if _database_url():
        ...
        with psycopg.connect(_database_url()) as conn:
            conn.execute("""
                ...
                (id, visitor_id, session_id, category, message, contact, page_context,
                 address_text, pnu_list, area_m2, recommendations, status, user_agent_group)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s,%s::jsonb,'open',%s)
            """, (
                feedback_id, data["visitor_id"], data.get("session_id"), data["category"],
...
def _feedback_rows(limit: int = 1000) -> List[Dict[str, Any]]:
    if _database_url():
        ...
        with psycopg.connect(_database_url()) as conn:
            rows = conn.execute("""
                SELECT id, created_at, updated_at, visitor_id, session_id, category, message,
            ...
            """, (limit,)).fetchall()
        keys = ["id","created_at","updated_at","visitor_id","session_id","category","message","contact","page_context","address_text","pnu_list","area_m2","recommendations","status","user_agent_group"]
        return [dict(zip(keys, row)) for row in rows]
    with ANALYTICS_LOCK:
...
def admin_dashboard(request: Request, _: bool = Depends(_admin_auth)):
    ...
    for r in analyses[:300]:
        ...
        table_rows.append(f"""
          <tr><td>{html.escape(str(created))}</td><td><code>{html.escape(str(r.get('visitor_id',''))[-10:])}</code></td>
    ...
    for r in feedback[:500]:
        ...
        feedback_rows.append(f"""
          <tr><td>{html.escape(str(created))}</td><td>{html.escape(category_labels.get(str(r.get('category')), str(r.get('category') or '-')))}</td>
    ...
    <h2>최근 대상지 분석</h2><div class="table"><table><thead><tr><th>시각</th><th>익명사용자</th><th>입력주소</th><th>면적</th><th>필지</th><th>추천결과</th><th>위치</th><th>PNU</th></tr></thead><tbody>{''.join(table_rows) or '<tr><td colspan="8">아직 분석 기록이 없습니다.</td></tr>'}</tbody></table></div>
    <h2>오류·개선의견</h2><div class="table"><table><thead><tr><th>접수시각</th><th>유형</th><th>내용</th><th>대상지</th><th>면적</th><th>연락처</th><th>처리상태</th></tr></thead><tbody>{''.join(feedback_rows) or '<tr><td colspan="7">접수된 오류·의견이 없습니다.</td></tr>'}</tbody></table></div>
    </main></body></html>"""
