...
def _query_building_hub_title(pnu: str) -> List[Dict[str, Any]]:
    ...
    while page <= 20:
        ...
        resp = requests.get(BUILDING_HUB_TITLE_URL, params=params, timeout=20)
        if resp.status_code >= 400:
            raise RuntimeError(f"건축HUB HTTP {resp.status_code}: {resp.text[:200]}")
        try:
            payload = resp.json()
        except Exception as exc:
            # Gateway/auth failures sometimes come back as XML/plain text.
            preview = resp.text[:300].replace("\n", " ")
            raise RuntimeError(f"건축HUB 비JSON 응답: {preview}") from exc

        items, total = _items_from_data_go_kr(payload)
        out.extend(items)
...
def _normalize_building_title(item: Dict[str, Any], pnu: str) -> Dict[str, Any]:
    keep = [
        ...
        "mainAtchGbCd", "mainAtchGbCdNm", "totArea", "platArea",
...
def health():
    return {
        ...
        "land_ledger": "ladfrlList_multi_fallback",
